...
template <> struct glz::meta<Context::General> {

    using T = Context::General;
...
GLZ_META(Context, camera, general, objects, version);
GLZ_META(Context::Camera, center, eye, up, paths);
GLZ_META(Context::General::OrthogonalProjection, height, width);
...
class Scene {
    ...
    Scene (const Context& context, const assets::FileSystem& fs);
