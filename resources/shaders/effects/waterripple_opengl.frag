...
void main() {
	...
	float mask = texture(g_Texture2, v_TexCoord.zw).r;
	
	vec3 n1 = texture(g_Texture1, v_TexCoordRipple.xy).xyz * 2 - 1;
	vec3 n2 = texture(g_Texture1, v_TexCoordRipple.zw).xyz * 2 - 1;
	vec3 normal = normalize(vec3(n1.xy + n2.xy, n1.z));
	
	texCoord.xy += normal.xy * g_Strength * g_Strength * mask;
	
	diffuseColor = texture(g_Texture0, texCoord);
	...
	diffuseColor.rgb += specular * g_SpecularColor * diffuseColor.a;
#endif
}
